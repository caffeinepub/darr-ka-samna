...
import { getCategoryByIdOrThrow, getCategoryLabel } from '../lib/categories';

export function CategoryPage() {
  const { categoryId } = useParams({ from: '/category/$categoryId' });
  const category = getCategoryByIdOrThrow(categoryId);
  const { data: stories = [], isLoading } = useStoriesByCategory(category.value);
  const categoryLabel = getCategoryLabel(category.value);

  return (
    <div className="container mx-auto px-4 py-8">
      {/* Category Header */}
      <section className="text-center py-12 mb-12">
        <h1 className="text-4xl md:text-6xl font-creepster text-destructive tracking-wider mb-4">
          {categoryLabel}
        </h1>
      ...
      {/* Category Stories */}
      <section>
        <h2 className="text-2xl font-creepster text-destructive mb-8 flex items-center gap-3">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 bg-card/30 rounded-lg border border-dashed border-border">
            ...
            <p className="text-lg text-muted-foreground">No stories in this category yet</p>
